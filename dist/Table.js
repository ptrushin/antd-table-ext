...
const Table = props => {
  ...
  const [tableColumnSettingsDialogState, setTableColumnSettingsDialogState] = useState({
    visible: false
  });
  const shiftKeyPressed = useMemo(() => ({
    pressed: false
  }), []);
  const getAble = (column, able) => {
    return rest[able] === false ? false : (column[able] === true || defaults && defaults[able] && column[able] !== false) === true;
  };
  const initColumns = () => {
    const columnsSortOrder = Object.assign({}, ...columns.filter(_ => _.defaultSortOrder || _.sortOrder).sort((a, b) => (a.sorter || {}).multiple - (b.sorter || {}).multiple).map((_, i) => ({
      [getColumnKey(_)]: i + 1
    })));
    for (let column of allColumns) {
      ...
      column.onHeaderCell = column => ({
        ...
        onInitWidth: width => columnResized({
          columnKey: column.key,
          width,
          init: true
        }),
      ...
      if (!column.children) {
        if (getAble(column, 'sortable')) {
          column.sorter = column.sorter === undefined ? {
            ...
            ...column.sorter,
            multiple: columnsSortOrder[column.key] || 0
          };
        }
      }
    }
  ...
  const columnMoved = ({
    ...
    forceUpdate();
    if (onColumnMoved) onColumnMoved({
      columnKey: from.key,
      toColumnKey: to.key
    });
    if (onColumnChange) onColumnChange({
      action: 'moved',
      columnKey: from.key,
      toColumnKey: to.key
    });
  ...
  const columnResized = ({
    ...
    forceUpdate();
    if (!init) {
      if (onColumnResized) onColumnResized({
        columnKey,
        width
      });
      if (onColumnChange) onColumnChange({
  ...
  const columnVisible = ({
    ...
    forceUpdate();
    if (onColumnVisible) onColumnVisible({
      columnKey,
      hidden
    });
    if (onColumnChange) onColumnChange({
  ...
  const columnFixed = ({
    ...
    forceUpdate();
    if (onColumnFixed) onColumnFixed({
      columnKey,
      fixed
    });
    if (onColumnChange) onColumnChange({
  ...
  const change = (newPagination, filters, sorter, props) => {
    ...
    forceUpdate();
    if (onChange) onChange(newPagination, filters, !sorter || !Array.isArray(sorter) ? sorter : sorter.sort((a, b) => (columnsMap[a.columnKey].sorter.multiple || 0) - (columnsMap[b.columnKey].sorter.multiple || 0)), props);
  };
      ...
      if (column.sorter) {
        column.title = ({
          ...
          return /*#__PURE__*/React.createElement("div", {
            ...
            className: "antd-ext-inner-title",
            title: column.description || titleText
          }, titleText), column.sorter.multiple > 0 && sortColumns && sortColumns.filter(_ => _.order).length > 1 && /*#__PURE__*/React.createElement("span", {
            style: {
              fontSize: 9,
              paddingTop: 0,
              verticalAlign: 'top'
...
const ResizableTitle = props => {
  ...
  if (!width || resizable === false) return /*#__PURE__*/React.createElement("th", _extends({
    ref: ref
  }, restProps));
