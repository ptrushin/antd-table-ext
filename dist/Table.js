...
const Table = ({
  ...
  useEffect(() => {
    if (!state) return;
    if (isInitialized) setStateToStorage(allColumns, stateStorable, state, history);else setIsInitialized(true);
  }, [state]);
  ...
  const getAble = (column, able) => {
    return rest[able] === false ? false : (column[able] === true || columnDefaults && columnDefaults[able] && column[able] !== false) === true;
  };
      ...
      if (!column.children) {
        if (getAble(column, 'sortable') || column.sorter || column.sortOrder || column.defaultSortOrder) {
          column.sorter = column.sorter ? {
            ...
            multiple: column.sorter.multiple || 0
          } : {
            multiple: 0
          };
  ...
  const afterStateToColumns = columns => {
    for (let column of columns) {
      ...
      if (column.sortable) {
        ...
        if (!column.sorter) column.sorter = {
          multiple: multiple
  ...
  const change = (newPagination, filters, sorter, props) => {
    ...
    if (props.action === 'filter') {
      for (let column of leafColumns) {
        ...
        if (column.filteredValue && !filter) delete column.filteredValue;else if (filter && column.filteredValue != filter) column.filteredValue = filter;
      }
    }
    columnsToState(setState, allColumns);
  ...
  const prepareColumns = (columns, level) => {
    ...
    for (let column of sortedColumns) {
      if (column.children) {
        ...
        column.onHeaderCell = column => ({
          ...
          onInitWidth: width => columnInitWidth({
            columnKey: column.key,
            width
          }),
          onContextMenu: event => {
        ...
        if (column.sortable) {
          column.title = ({
            ...
            return /*#__PURE__*/React.createElement("div", {
              ...
              className: "antd-ext-inner-title",
              title: column.description || titleText
            }, titleText), column.currentSortIndex > 0 && sortColumns && sortColumns.filter(_ => _.order).length > 1 && /*#__PURE__*/React.createElement("span", {
              style: {
                fontSize: 9,
                paddingTop: 0,
                verticalAlign: 'top'
              }
            }, column.currentSortIndex));
          };
        } else {
          let titleText = title instanceof Function ? title() : title;
          column.title = ({
    ...
    if (level === 0) {
      ...
      if (addLastColumn && !hasProcentColumns) tableColumns.push({
        width: '100%'
      });
...
const exportTable = /*#__PURE__*/React.forwardRef((props, ref) => {
...
const ResizableTitle = props => {
  ...
  if (!ifWidthInPixels || resizable === false) return /*#__PURE__*/React.createElement("th", _extends({
    ref: ref,
    width: width
  }, restProps));
