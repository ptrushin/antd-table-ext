...
export const getTreeLeafColumns = (treeColumns) => {
    ...
    for (let column of treeColumns) {
        if (!column.children || column.children.length === 0) columns.push(column);
        else columns = [...columns, ...getTreeLeafColumns(column.children)];
    }
    return columns;
}
...
export function getRecordValue(record, dataIndex) {
    const getValue = (record, dataIndexArr, level = 0) => {
        return level === dataIndexArr.length
            ? record
            : getValue(record[dataIndexArr[level]], dataIndexArr, level + 1);
    }
